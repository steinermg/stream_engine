...
class StreamDataset(object):
        ...
        if source_key != self.stream_key:
            ...
            for deployment, ds in self.datasets.items():
                if new_name in ds:
                    continue
                try:
                    interpolated_var = source_dataset.get_interpolated(ds.time.values, parameter,
                                                                   deployment if only_same_deployment else None)
                    if interpolated_var:
                        ds[new_name] = interpolated_var
                    else:
                        log.info('<%s> stream_dataset::interpolate_into: Could not interpolate %r from (%s) into (%s)',
                                 self.request_id, parameter, source_key.as_refdes(), self.stream_key.as_refdes())
    ...
    def get_interpolated(self, target_times, parameter, required_deployment=None):
        ...
        if datasets:
            ...
            log.info('<%s> stream_dataset::get_interpolated: %r was not found in datasets from source (%s)',
                     self.request_id, parameter, self.stream_key.as_refdes())
